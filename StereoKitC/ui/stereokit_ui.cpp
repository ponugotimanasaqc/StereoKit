...
void ui_button_behavior(vec3 window_relative_pos, vec2 size, ui_hash_t id, float &finger_offset, button_state_ &button_state, button_state_ &focus_state) {
	...
	if      (button_state & button_state_just_active)   sound_play(skui_snd_interact, skui_interactors[interactor].hit_test_world.at, 1);
	else if (button_state & button_state_just_inactive) sound_play(skui_snd_uninteract, skui_interactors[interactor].hit_test_world.at, 1);
}
...
button_state_ ui_volumei_at_g(const C *id, bounds_t bounds, ui_confirm_ interact_type, handed_ *out_opt_hand, button_state_ *out_opt_focus_state) {
	...
	result = ui_interactor_set_active(interactor, id_hash, active, interact_at);
...
bool32_t ui_volume_at_g(const C *id, bounds_t bounds) {
	...
	for (int32_t i = 0; i < skui_interactors.count; i++) {
		...
		float         interact_dist;
		vec3          interact_at;
		bool          in_box = ui_interact_box(actor, size, &interact_at, &interact_dist);
...
button_state_ ui_interact_volume_at(bounds_t bounds, handed_ &out_hand) {
	...
	for (int32_t i = 0; i < skui_interactors.count; i++) {
		...
		float priority = 0;
		vec3 at;
		if (ui_interact_box(&skui_interactors[i], bounds, &at, &priority)) {
...
bool32_t ui_hslider_at_g(const C *id_text, N &value, N min, N max, N step, vec3 window_relative_pos, vec2 size, ui_confirm_ confirm_method) {
	...
	if (confirm_method == ui_confirm_push) {
		...
		if (focus_state & button_state_active) {
			...
			button_state = ui_interactor_set_active(actor, id, false, interaction_at);
		}
		if (actor != -1)
		...
		if (actor != -1) {
			...
			button_state = ui_interactor_set_active(actor, id, pinch & button_state_active, interaction_at);
			// Focus can get lost if the user is dragging outside the box, so set
			// it to focused if it's still active.
